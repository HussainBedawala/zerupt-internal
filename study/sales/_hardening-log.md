...
| 3 | ⏳ next | | 0132 | Audit banked (/tmp/sales-hardening/layer-3-audit.md): VAT/AR/revenue VERIFIED correct (balanced JE, party-tagged, net-of-discount, period-gated). G1 CRITICAL = invoice VOID absent (only credit note today) → build now mirroring purchase voidBill: contra revenue+output-VAT+AR off stored amounts, reuse sale_return restock+COGS-reversal at engine-realized cost [L2-tie], 409 once paid/credited, PIN+SoD, period-validated, outbox; needs migration (add 'voided' status + void cols). G2 = priceOverrideById silently dropped → 422 reject-guard now, defer real lookup. Rest LOW/defer. |
