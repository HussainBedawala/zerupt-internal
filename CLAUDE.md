...
- **Bidi isolation** — wrap user content of unknown direction in the native `<bdi>` element (the house convention, ~193 files). There is no `lib/bidi.ts` helper; a nested `<bdi>` isolates a Latin document number inside an Arabic sentence. Pinned by `apps/web/src/__tests__/bidi-navigation-separator-guard.test.ts`
...
- **A nowrap label inside a Radix ScrollArea viewport grows past the gutter** — the Viewport wraps children in a `display:table` div that sizes to max-content, so a `whitespace-nowrap` label pushes the row's controls (chevron, new-tab icon) under the absolutely positioned scrollbar; labels must be `min-w-0 truncate`, and any `<a>` holding icon + label must itself be `flex`
