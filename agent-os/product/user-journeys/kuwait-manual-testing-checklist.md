...
created: 2026-06-09
audited_against: stress-test-master-journeys.md (2026-06-09 green pass)
purpose: >
...
- [ ] **115.** You see 4 cards labelled "**How do you print receipts?**":
  1. "Thermal printer (80mm roll)"
  2. "Thermal printer (58mm roll)"
  3. "Regular A4 printer"
  4. "No printer, email receipts"
...
> This is where Yousef imports his existing data. Mira (the AI) assists with mapping.

- [ ] **148.** The heading reads "**Bring in your data**".
- [ ] **149.** Below it: "Choose what to import now. You can always finish the rest later from your dashboard."
- [ ] **150.** You see 8 import options, each with a Yes/No choice:
  1. **Categories** — "How your products are grouped — the shelves your items sit on."
  2. **Products** — "Your item list — names, prices, and barcodes."
  3. **Customers** — "Your customer list — names, phone numbers, and what they owe you."
  4. **Suppliers** — "Who you buy from — names, contacts, and what you owe them."
  5. **Opening Stock** — "How much of each item you have on hand right now, and what it's worth."
  6. **Trial Balance (Opening Balances)** — "Export a Trial Balance from your current system — we'll match it to your accounts."
  7. **Customer Outstanding (AR Aging)** — "Who owes you money and how much, per invoice — powers your AR aging reports."
  8. **Supplier Outstanding (AP Aging)** — "Who you owe money to and how much, per invoice — powers your AP aging reports."
- [ ] **151.** A status indicator near the top shows Mira's status — it should say "**Mira is on the job — AI-assisted import ready**" (or "Mira is offline — manual mode active" if AI is down).
- [ ] **152.** For each of the 8 options, click "**Yes, bring them in**" → a dialog opens → upload the corresponding file from `test-data/yousef/`. Verify Mira maps the columns and shows a preview. Check for error messages on bad rows. Apply the import.
- [ ] **153.** ⚠️ When uploading the **products file** (3,000 SKUs), watch: Mira should flag bad rows (orphan SKUs, duplicates, zero-qty items) as separate decisions — not block the entire import.
- [ ] **154.** ⚠️ When uploading the **customers file** — try the **Windows-1256 Arabic CSV** (`09-customers-windows1256.csv`). Arabic names must render correctly — no garbled characters (e.g. `???` or random symbols).
- [ ] **155.** ⚠️ After the products import, upload the **IMEI register** (900 serials) as part of the opening stock or via the serial import path. Serials should land and be tied to SKUs.
- [ ] **156.** After completing or skipping imports, click "**Continue to go-live**".
...
- [ ] **157.** The heading reads "**You're ready to go live**".
- [ ] **158.** Below the heading: "One last check, then your business is open. Your team gets access and you can start working with real data."
- [ ] **159.** A "**Go-live checks**" section shows checklist items. Each item is marked "Required" or "Recommended". All "Required" items must be green/passed before you can go live.
- [ ] **160.** If you imported a trial balance that doesn't fully tie, a "**Opening balances**" reconciliation section appears showing the variance. ⚠️ Yousef's messy import will have a variance — the system should show him the difference in KWD (3 decimal places) and ask him to acknowledge it before going live.
- [ ] **161.** An acknowledgement checkbox appears: "I understand my opening balances are incomplete and I choose to go live anyway." — Yousef must tick this to unlock the Go Live button.
- [ ] **162.** Click "**Go live**". A confirmation dialog appears: "Go live now?" with body text mentioning invitations (if any team invites were set) and the warning "You can't undo this."
- [ ] **163.** Click "**Yes, go live**". The system goes live.
- [ ] **164.** ⚠️ After going live, attempt to access the onboarding wizard again. It should NOT be accessible — you should be redirected to the dashboard. "Your business is already live. Taking you to your dashboard…"
- [ ] **165.** 💚 **The emotional test:** At this point Yousef should feel "My shop is live." The dashboard should load with real data, currency showing KWD with 3 decimal places everywhere, and no tax lines anywhere.
...
- [ ] **166.** Start a fresh onboarding session. Fill in Step 1 (business name, Kuwait, industry). Do NOT click Continue. **Close the browser tab entirely.** Re-open the app and navigate back to onboarding. Step 1 should still show your entries — the wizard auto-saves.
- [ ] **167.** Go back to Step 1 from Step 3 using the "Back" button. The Step 1 fields should all still be filled in.

**RTL / Arabic layout test**
- [ ] **168.** In Step 1, set the preferred language to "Arabic" and confirm the interface-switch dialog. The entire wizard should reload in Arabic, with the layout mirrored right-to-left (text flows right, inputs are right-aligned, the "Back" button moves to the right side).
- [ ] **169.** Switch back to English and verify the layout mirrors back to left-to-right.

**Validation red-path tests**
- [ ] **170.** In Step 2, try clicking Continue with a branch card whose name is blank. An error message should appear under the branch name field.
- [ ] **171.** In Step 2, try clicking Continue with a branch card whose name is a duplicate of another branch. An error like "This name is already used" should appear.
- [ ] **172.** In Step 1, try typing letters in the "Years operating" field. The field should either reject non-numeric input or show a validation error.

**Step locking test**
- [ ] **173.** Try clicking on a later step (e.g. Step 5 — Team) in the sidebar/progress bar before completing Step 1. It should be locked ("Complete previous steps to unlock") and not navigable.

**KWD decimal watch — everywhere in onboarding**
- [ ] **174.** At any point during the wizard, if you see a money amount displayed (e.g. in the review screen, in an import preview, in an opening balance variance) — it must show **3 decimal places** (e.g. `1,250.750 KWD`). If you ever see 2 decimals (`1,250.75`) — stop and log it.
...
| ⚠️ **Customer opening balances not importable** | Try to import reseller/neighbour dues | Balances should land on the customer + AR |
