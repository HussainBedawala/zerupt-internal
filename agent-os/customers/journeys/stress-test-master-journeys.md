...
- ✅ **BUILT (PR #151, 2026-06-11)** ~~⏭️ DEFERRED~~ **ZATCA Phase 1+2** (TLV QR + cleared XML + XAdES-B-B signing + EGS onboarding + 5-yr archival) is **built** on branch `phase-2/zatca-einvoicing`, gated `ZATCA_ENABLED`+KSA-only. ~~"0% built / KSA cannot legally transact" was the pre-PR verdict — reversed.~~ Remaining: live sandbox HTTP round-trip (MORNING-BRIEF §2), Z3/Z9 test gates. *(Gap #2 — now conditionally ready.)* **Saudi has NO basic-food zero-rating** — every baqala SKU is 15% (correct as-is, nothing to seed).
...
- ✅ **BUILT (PR #151)** ~~⏭️ DEFERRED~~ KSA `zatcaEnabled` toggle is now wired to `tenant_identity` and read by the ZATCA module gate. *(Gap #2 — reversed.)*
...
- ✅ **BUILT (PR #151)** ~~⏭️ DEFERRED~~ **KSA:** TLV QR on B2C receipts now built (tags 1-9 server-signed preferred, tags 1-5 client fallback); simplified → reporting queue; standard → clearance (blocking). *(Gap #2 — reversed.)* Remaining: sandbox HTTP round-trip validation.
...
## Stage 10 — Month-End & Compliance · 🟢 *(green for all VAT countries; KSA ZATCA now built, pending sandbox validation)*
...
- ✅ **BUILT (PR #151, 2026-06-11)** ~~⏭️ DEFERRED~~ **KSA:** ZATCA Phase 1+2 is built (clearance + reporting + signed XML archival + EGS onboarding). ~~"0% built / do not sell there yet" was the pre-PR verdict — reversed.~~ Remaining: sandbox HTTP validation (MORNING-BRIEF §2) + Z3/Z9 test gates. *(Gap #2 — conditionally ready.)*
- ✅ **FIXED (2026-06-09)** ~~🔴~~ **Bahrain:** zero-rated (Rx) tax group **now seeded** (→ VAT-BH-0) → Rx no longer defaults to 10%. *(Gap #12.)*
- ✅ **By design (non-blocking):** no e-invoicing connectors for non-KSA countries — **no country except KSA has a live retail e-invoicing mandate in 2026** (UAE B2C not yet, QA draft, BH/OM no live mandate, Oman Fawtara large-taxpayers only). Read-only VAT reports + manual transcription is compliant everywhere we sell. KSA ZATCA is a hard mandate, and that's now built.
...
## 🇸🇦 Saudi — *3 personas: Abu Khalid (Riyadh auto-parts, B2B clearance), Umm Saud (Jeddah baqala, B2C reporting), Al-Faisaliah Mobiles (Dammam electronics, mixed + credit notes)*

- **Triggers:** Abu Khalid — ZATCA enforcement wave + SAR 12k retrofit quote. Umm Saud — inspector checked QR on her receipts. Al-Faisaliah — corporate client demands ZATCA-cleared invoice.
- **ZATCA status (2026-06-11): ✅ BUILT** on PR #151 (`phase-2/zatca-einvoicing`), gated `ZATCA_ENABLED` + KSA-only. ~~Earlier "0% built" / "deferred blocker" verdicts are OUTDATED and reversed.~~ Remaining gaps are narrow (buyer National Address schema, per-register CSID POS picker, live sandbox HTTP validation — see `ksa-customer-journey.md` Part 3).
- **Deltas:**
  - ✅ **BUILT (PR #151)** **ZATCA Phase 1+2**: TLV QR (tags 1-9), UBL 2.1 XML, XAdES-B-B signing (secp256k1), PIH chain, EGS onboarding wizard (OTP→CCSID→compliance samples→PCSID), clearance (standard/blocking) + reporting (simplified/24h pg-boss), 5-yr encrypted archive, `zatcaEnabled` wired to `tenant_identity`. ~~Was ⚫ 0% — reversed.~~
  - ✅ **BUILT** S/Z/E/O VAT category mapping with **fail-closed** on unknown exemption (`ZatcaMissingExemptionError`).
  - ✅ **BUILT** Non-SAR invoices **rejected** at ZATCA boundary (USD quotes → explicit error, not silent SAR conversion).
  - ✅ **BUILT** Credit notes (381) with `BillingReference` to original + same invoice subtype.
  - ✅ **Correct as-is** 15% VAT, NO food zero-rating — every SKU 15%; SAR (2dp).
  - ✅ **FIXED** ~~🟡~~ **OEM/part-number field** — `part_number` item attribute added. *(Gap #25.)*
  - ✅ **FIXED** ~~⚫~~ **PDC** — full cheque lifecycle built. *(Gap #21.)*
  - 🔴 **Buyer National Address** on B2B standard invoices — deferred follow-up (customer address schema).
  - 🟡 **EGS-per-register POS picker** — schema ready; register CSID UI in POS deferred.
  - 🔴 **Sandbox HTTP live validation** — MORNING-BRIEF §2 outstanding (CCSID issuance + clearance/reporting round-trip vs ZATCA servers).
- **Verdict: 🟡 Conditionally ready — ZATCA is built; sandbox E2E validation + Z3/Z9 test gates outstanding.** Once MORNING-BRIEF §2 is run and Z3/Z9 pass, KSA can accept pilot tenants. See `ksa-customer-journey.md` §Part 4 for the full pre-go-live checklist.
...
> **Status snapshot — last audited 2026-06-11 (ZATCA PR #151 built).** Of 27 gaps: **26 ✅ done · 1 🟡 conditionally ready · 1 ⏭️ deferred-by-decision.** Gap #2 (ZATCA) is now built on PR #151 — reversed from "deferred." The **only** remaining non-green item is Urdu/Hindi locale (#24). Gap #2 is conditionally ready pending sandbox HTTP round-trip validation.

### 🟡 Conditionally ready (code built, validation outstanding)

| # | Gap | State | What remains |
|---|-----|-------|--------------|
| 2 | **ZATCA Phase 1+2** | 🟡 Built, pending validation | Code complete PR #151 (`phase-2/zatca-einvoicing`), byte-exact vs ZATCA SDK offline. Remaining: (a) live sandbox HTTP round-trip — MORNING-BRIEF §2 (CCSID issuance + clearance/reporting vs fatoora.zatca.gov.sa sandbox); (b) Z3 (VAT-registered blank-TRN block on standard invoice) test pass; (c) Z9 (garbled exemption fail-closed) test pass against umm-saud dataset. Once these three pass, KSA accepts pilot tenants. |

### ⏳ Deferred by decision (one item)
...
# PART 5 — KSA Adversarial Stress-Test Suite (ZATCA Z1-Z18 + Messy Datasets)

> Updated 2026-06-11 to reflect PR #151 built reality. These are adversarial "try to break it" scenarios, not happy-path checks. Frame each as: **setup → action → expected system behavior**. Run against fresh tenants from `test-data/{persona}/` per `KSA-PERSONAS-SPEC.md`.

**Personas:** Abu Khalid (B2B clearance, Riyadh), Umm Saud (B2C reporting, Jeddah), Al-Faisaliah Mobiles (mixed + credit notes, Dammam).

---

## §KSA-1 — TRN / Seller VAT Validation (Z1–Z5, Z4: onboarding)

| Scenario | Persona | Setup | Action | Expected behavior |
|---|---|---|---|---|
| **Z1: 14-digit TRN** | Al-Faisaliah | `03-customers.csv` has two customers with 14-digit TRNs | Import customers | Import surfaces validation error per row; does NOT silently accept or truncate |
| **Z2: TRN not ending in 3** | Al-Faisaliah | Customer TRN `301234567890124` (last digit ≠ 3) | Import or inline create | Validation error with clear format hint: "15 digits, first and last must be 3" |
| **Z3: VAT-registered + blank TRN on standard invoice** | Abu Khalid + Al-Faisaliah | Customer flagged `VAT Registered = Yes` but `TRN` column empty | Confirm a standard (B2B) invoice to this customer | System blocks or hard-flags per BR-KSA-42/81; invoice must NOT proceed to clearance with missing buyer TRN |
| **Z4: Seller VAT fails 3..3 format** | Any | In Settings, enter seller VAT `123456789012345` (doesn't start with 3) | Save | Settings validation rejects with format error |
| **Z5: National Address incomplete** | Any | Enter seller VAT correctly but leave building no blank | Attempt to enable ZATCA or create EGS unit | Onboarding completeness gate blocks with specific missing-field message |

---

## §KSA-2 — VAT Category Mapping + Exemption Fail-Closed (Z6–Z9, Z14–Z15)

| Scenario | Persona | Setup | Action | Expected behavior |
|---|---|---|---|---|
| **Z6: taxGroup string mismatch** | Umm Saud + Abu Khalid | `02-products.csv` has row: `taxGroup = "Standard Rate 15%"` (no parentheses, vs canonical form) | Import products | System surfaces mismatch warning; does NOT silently assign wrong rate; human must confirm mapping |
| **Z7: Zero-rated item** | Umm Saud | Product with `taxGroup = Zero Rate` (e.g. export/medicine) | Add to sale → issue invoice | ZATCA XML emits Category Z + valid VATEX code; `cbc:TaxableAmount` + 0 tax amount; QR reflects correctly |
| **Z8: Exempt item** | Umm Saud | Product with `taxGroup = Exempt` | Add to sale → issue invoice | Category E + valid exemption reason text; exempt amount reported correctly |
| **Z9: Garbled / unknown exemption** | Umm Saud | Product row with `taxGroup = "SPECIAL-EXEMPT-XYZ"` (not a recognized code) | Add to sale → attempt invoice | **`ZatcaMissingExemptionError` thrown** — invoice blocked; error surfaces in bilingual UI; system does NOT emit a wrong or blank exemption code to ZATCA |
| **Z14: Multi-line S+Z+E basket** | Umm Saud | Cart with 3 lines: 15% standard, 0% zero-rated, exempt item | Complete POS sale | ZATCA XML shows correct grouped VAT breakdown per category; totals sum correctly; half-up 2dp rounding at doc level |
| **Z15: Discount + rounding midpoints** | All | Line item SAR 10.005 after discount | Issue invoice | VAT rounded half-up to 2dp; doc-level VAT rounded once (not per-line then re-summed); test against known SAR fixture |

---

## §KSA-3 — Non-SAR / FX Rejection (Z10)

| Scenario | Persona | Setup | Action | Expected behavior |
|---|---|---|---|---|
| **Z10: USD-priced invoice** | Al-Faisaliah | Create a sales invoice with line amounts in USD (corporate UAE client quote) | Attempt ZATCA clearance or reporting | System rejects at ZATCA boundary with explicit "non-SAR invoices are not accepted" error; does NOT convert USD→SAR silently and submit a wrong amount; invoice stays in a blocked/non-submitted state |

---

## §KSA-4 — Simplified B2C Reporting + TLV QR (Z11, Z16)

| Scenario | Persona | Setup | Action | Expected behavior |
|---|---|---|---|---|
| **Z11: B2C POS receipt QR** | Umm Saud | Complete a cash/mada POS sale (simplified invoice path) | Print or display receipt | TLV QR is present; tags 1-5 minimum (Phase 1); tags 1-9 preferred (Phase 2, server-signed); scan QR with ZATCA validator → valid |
| **Z11 high-volume**: 500 receipts in quick succession | Umm Saud | Simulate a busy Friday queue (500 sales, 1-min window) | Bulk POS sales | Reporting queue processes all within 24h SLA; no receipts lost in dead-letter; ICV counter has no gaps |
| **Z16: Offline sale → reconnect** | Umm Saud | Kill network (DevTools offline), ring up 10 sales | Restore network | Offline-replay queue submits all 10 to reporting within 24h; each gets a `zatca_invoice_documents` row with final status |
| **Z16 replay idempotency** | Umm Saud | Replay the same offline batch twice (simulated double-sync) | Queue processes | Each invoice posted exactly once; `(tenant, clientId)` dedup → no duplicate ZATCA submissions |

---

## §KSA-5 — B2B Standard Clearance (Z12)

| Scenario | Persona | Setup | Action | Expected behavior |
|---|---|---|---|---|
| **Z12: Happy path clearance** | Abu Khalid | Garage customer with valid 15-digit TRN, seller VAT 3..3, EGS onboarded | Confirm standard B2B invoice | Clearance submitted synchronously (blocking); `zatca_invoice_documents.status = cleared`; signed XML archived; buyer copy valid only after clearance |
| **Z12: Clearance rejection** | Abu Khalid | ZATCA sandbox returns a 400 (e.g. invalid TRN format, cert mismatch) | Confirm standard invoice | System surfaces bilingual actionable error; invoice stays in `rejected` state; ICV still consumed + PIH chain continues to next invoice; human can correct and retry |
| **Z12: Clearance timeout** | Abu Khalid | ZATCA API unreachable (network drop) | Confirm standard invoice | Durable queue retries with backoff; dead-letter surfaces after N failures; alert fires; invoice in `pending` not `cleared` |
| **Z12: Concurrent clearance** | Abu Khalid | Two B2B invoices confirmed simultaneously | Race condition | ICV counter advisory lock per EGS unit → sequential ICV/PIH chain; no gaps, no duplicates |

---

## §KSA-6 — Credit Notes + Return Chains (Z13)

| Scenario | Persona | Setup | Action | Expected behavior |
|---|---|---|---|---|
| **Z13: Partial return of a cleared B2B invoice** | Al-Faisaliah | Original standard invoice cleared (status=cleared); customer returns 1 of 3 phones | Issue credit note 381 | Credit note carries `BillingReference` to original invoice's UUID + ICV; same invoice subtype (standard); VAT fully reversed for returned lines; credit note goes through clearance path |
| **Z13: Credit note on a simplified invoice** | Al-Faisaliah | Original B2C simplified sale; warranty exchange | Issue credit note | Credit note subtype = simplified; goes through reporting (not clearance); QR on credit note document |
| **Z13: Chain — credit note then new invoice** | Al-Faisaliah | Phone returned → replacement issued | Credit note 381 + new standard sale | ICV chain: original (N) → credit note (N+1) → new invoice (N+2); PIH(N+2) = hash(N+1); no gaps |

---

## §KSA-7 — Tally TB Reconciliation Gate (Z17)

| Scenario | Persona | Setup | Action | Expected behavior |
|---|---|---|---|---|
| **Z17: Unbalanced Tally TB** | Abu Khalid | `07-trial-balance.csv` has SAR ~890 debit/credit gap (deliberate) | Upload TB → attempt go-live | Go-live readiness gate surfaces imbalance with SAR amount; requires explicit checkbox acknowledgement ("I understand my opening balances are incomplete"); server rejects go-live without acknowledgement |
| **Z17: Clean TB after reconciliation** | Abu Khalid | User edits TB to fix gap → re-imports clean version | Re-check readiness | Imbalance warning gone; go-live proceeds |

---

## §KSA-8 — PDC Bounce Workflow (Z18)

| Scenario | Persona | Setup | Action | Expected behavior |
|---|---|---|---|---|
| **Z18: Cheque matures — collected** | Abu Khalid | `06-pdc-register.csv` PDC rows, maturity date arrives | Mark collected | PDC lifecycle: `PDC Receivable → AR cleared`; accounting JE posted; no alert |
| **Z18: Cheque bounces** | Abu Khalid | PDC row with `BOUNCE` flag in dataset | Mark bounced | Bounce workflow: reversal JE posted; `PDC Receivable → written-off or re-opened`; alert surfaces to owner; AR balance restored |
| **Z18: Multiple PDCs for same garage** | Abu Khalid | Garage has 3 outstanding PDCs at different maturities | View PDC register | All 3 visible with maturity dates; total PDC balance ties to AR aging for that customer |

---

## §KSA-9 — Combined Gnarliest Journeys (multi-system stress)

These test the interaction between ZATCA, the import pipeline, and the accounting engine simultaneously. Each one tries to make the system do something it shouldn't.

### KSA-COMBO-1: "Bad TRN garage gets a clearance invoice"
Abu Khalid imports `03-customers.csv` with a garage having a 14-digit TRN (Z1). He then tries to sell to that garage. **Expected:** the import flags the bad TRN; if user forces the customer through anyway, the standard invoice confirm path must block or hard-warn at the ZATCA clearance step before submission.

### KSA-COMBO-2: "Garbled exemption floods the reporting queue"
Umm Saud imports `02-products.csv` — one product has garbled `taxGroup`. She sells that product to 50 walk-in customers. **Expected:** at sale time, the unknown exemption fails-closed (`ZatcaMissingExemptionError`) on each transaction; the reporting queue does NOT receive 50 malformed simplified invoices; the dead-letter shows 50 failures with a clear error, not 50 garbage XML submissions to ZATCA.

### KSA-COMBO-3: "B2C reporting backlog + offline replay race"
Umm Saud's shop goes offline for 8 hours (internet outage). She processes 200 POS sales offline. She reconnects. Simultaneously the 24h sweeper fires. **Expected:** offline replay and sweeper cooperate — each reporting job is idempotent (`(tenant, posTxnId)` dedup); ZATCA receives exactly 200 reports, not 400; all get `status = reported`; ICV sequence has no gaps.

### KSA-COMBO-4: "USD corporate quote slips into the clearance queue"
Al-Faisaliah creates a USD-denominated invoice for a UAE corporate client (Z10). A misconfigured currency import puts the FX rate as 1.00. The invoice passes internal validation. **Expected:** ZATCA boundary rejects the non-SAR invoice before any XML is submitted; the `zatca_invoice_documents` row is in `rejected` state; the `TaxCurrencyCode` check catches it early, not after an XAdES sign attempt.

### KSA-COMBO-5: "Credit note chain breaks the PIH sequence"
Al-Faisaliah posts invoice N (cleared), then credit note N+1 (cleared), then invoice N+2 (cleared). Concurrently, another browser tab tries to confirm a second invoice simultaneously with N+2. **Expected:** ICV counter advisory lock serializes the two; one gets ICV=N+2 and the other ICV=N+3; PIH chain is contiguous; no two invoices share the same ICV; ZATCA would accept the full sequence.

### KSA-COMBO-6: "Tally TB doesn't balance + seller VAT wrong → can't go live"
Abu Khalid attempts go-live with: (a) unbalanced TB AND (b) seller VAT not yet entered in Settings. **Expected:** readiness gate shows both blockers (imbalance + missing seller VAT); the ZATCA enable is greyed out until seller VAT 3..3 is saved; TB imbalance requires explicit acknowledgement; neither issue silently passes.
