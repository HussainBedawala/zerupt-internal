...
>
> **Audit note (2026-06-11):** ZATCA implementation merged to `main` on 2026-06-11 (60 files under `apps/api/src/zatca/` plus frontend under `apps/web/src/features/zatca/`). All features below are marked `shipped` except one: PCSID Renewal and CRL Handling, which remains `planned` because `renewProductionCsid` exists only as an API-client method with no onboarding service or controller wiring, and CRL handling has no implementation at all.
